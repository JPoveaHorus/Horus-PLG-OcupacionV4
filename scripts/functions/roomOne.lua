...
local devicesOff = _storage.get_table("ACTUADORES_OFF1") or {}
...
local masterswitch = _storage.get_string("MASTERSWITCH1") or {}
...
function ActualizarAccion(s)
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Verificar("accionTexto1", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Verificar("statusText1", statusText)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        if modoStatus then
            Verificar("Mode1", tostring(modoStatus.value))
        end

        if _storage.get_number("accion1") == ACCION_LIBRE  or  _storage.get_number("accion1") == ACCION_CICLO_APAGADO then
            Rutina_Encendido()
        end

        ActualizarAccion(ACCION_PABIERTA)
        Verificar("timerAccion1", "AutoOff")
        Verificar("scanCycle1", "AutoOff")
    ...
    if SensorPuerta.value == false then
        CancelTimer()
        if _storage.get_number("accion1") == ACCION_PABIERTA then

            if _storage.get_string("statusText1") == "Libre" then
                Rutina_Encendido()
            end

            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle1", "scan_1")
            Verificar("timerAccion1", "Scan")
            StartTimer(Tiempo1)
            return true
        else
        ...
        for i in ipairs(Actuador) do
            ...
            if actuador_id.value == true then
                if data.scancycle == nil or data.scancycle == "Libre" then
                    ...
                    Verificar("scanCycle1", "Ocupado")
                    ActualizarAccion(ACCION_OCUPADO)
                    Registro("Pasa a accion 1 OCUPADO por disparo de Actuador true", amarillo)
                    return
                end
                if _storage.get_number("accion1") == ACCION_PABIERTA and data.scancycle == "PuertaAbierta" then
                    if _storage.get_string("statusText1") == "Libre" then
                        if masterswitch == "si" then
                            RoutineOn(devicesOn)
                        end
                ...
                if (tiempoRestante1 >= 10 and data.scancycle == "scan_1") then
                    ...
                    if _storage.get_number("accion1") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion1", "Ocupado")
                        Verificar("scanCycle1", "Ocupado")
                        Registro("se paso a accion1 OCUPADO por disparo de Actuador", verde)
                        return
                    end
                end
            ...
            if actuador_id.value == false then
                if _storage.get_number("accion1") == ACCION_SCAN then
                    ...
                    Verificar("timerAccion1","Ocupado")
                    Verificar("scanCycle1", "Ocupado")
                    Registro("Pasa a accion 1 OCUPADO por disparo de Actuador false", amarillo)
                    return
                end              
                if _storage.get_number("accion1") == ACCION_PABIERTA and data.statustext == "Ocupado" then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Verificar("timerAccion1", "Scan")
                    Verificar("scanCycle1", "scan_1")
...
function PuertaAbierta()
    ...
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Verificar("timerAccion1", "PuertaAbierta")
    Verificar("scanCycle1", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then
        ...
        --     offpuertaabierta .. ", " .. _storage.get_string("Mode1")
...
function CycleOff()
    ...
    if (contador == contadorLibres) then
        if data.scancycle == "scan_1" then
            CancelTimer()
            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle1", "scan_Libre")
            Verificar("timerAccion1", "Scan")
            StartTimer(Tiempo2)
        ...
        elseif data.scancycle == "scan_Libre" then
            ...
            Verificar("timerAccion1", "CicloApagado")
            Verificar("scanCycle1", "CicloApagado")
...
function Rutina_Encendido()
    ...
    local mode = _storage.get_string("Mode1")
...
function RoutineOn(item_id)
    local id = {}
        if _storage.get_string("Mode1") == MODO_AUTO then
...
function Main()
    ...
    Registro("previousTimer_1: " .. _json.encode(data.previousTimer))
    Registro("TimerRemainingV_1: " .. _json.encode(TimerRemainingV))
    Registro("Modo plugin: " .. _json.encode(data.modo),verde)
    Registro("TimerAccion1: " .. _json.encode(data.timeraccion),amarillo)
    ...
    Verificar("previousTimer1", tonumber(TimerRemainingV))

    if data.accion == 100 or data.accion == 0 then
        Verificar("TimerDuration1", 0)
    else
...
function StartTimer(timerduration)
    ...
    Verificar("TimerDuration1", timerduration)
...
function StartTimeralways()
    ...
    Verificar("dueTimestamp1", dueTimestamp1)
    local status = RemainingUpgrade()
    Verificar("Counting1", "1")
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID1", tostring(timerID))
        --Registro("TimerID1: " .. timerID) -- No es necesario mostrar el ID del Timer
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/functions/roomOne",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/functions/roomOne",
            { arg_name = "timer" })
        _storage.set_string("TimerID1", tostring(timerID))
        return true
    end
...
function CancelTimer()
    ...
    if data.TimerID ~= "" then
        local timerID = data.TimerID
        _storage.set_string("TimerID1", tostring(timerID))
        --Registro("TimerID1: " .. timerID, amarillo) -- No es necesario mostrar el ID del Timer
        if timerID then
    ...
    Remaining_ant = 0
    Verificar("Counting1", "0")
    Verificar("dueTimestamp1", 0)
    Verificar("remaining1", "0")
    Verificar("TimerDuration1", 0)
    return true
end
...
function RemainingUpgrade()
    ...
    Verificar("remaining1", "TR" .. restante)
    return remaining > 0
end
...
if modoPluginStatus then
    ...
    if modeButton.value ~= data.modo then
        Verificar("Mode1", tostring(modeButton.value))
...
if AccionPluginStatus then
    ...
    if AccionButton.value == "Libre" then
        if EstadoTexto.value == "Ocupado" then
            ...
            Verificar("scanCycle1", "LibreForzado")
            Libre()
        end
    end
end
