...
local sensor_puerta = _storage.get_string("SENSOR_PUERTA7")
local devicesOn = _storage.get_table("ACTUADORES_ON7")
local devicesOff = _storage.get_table("ACTUADORES_OFF7") or {}
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT7")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC7")
local thermostat = _storage.get_table("TERMOSTATO7")
local idmasterswitch = _storage.get_string("MASTERSWITCH7")
...
local offpuertaabierta = _storage.get_string("OFFPUERTAABIERTA7")
local masterswitch = _storage.get_string("MASTERSWITCH7") or {}
local motionActivator = _storage.get_string("MOTIONACTIVATOR7")
local SetPointOn = _storage.get_number("SETPOINTON7")
local SetPointOff = _storage.get_number("SETPOINTOFF7")
local ModoSetpoint = _storage.get_string("MODOSETPOINT7")

local data = {}
data.modo = _storage.get_string("Mode7")
...
----- Tiempos del Plugin
local Tiemposensor = _storage.get_number("TIEMPOSCAN7")
...
function ActualizarAccion(s)
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Verificar("accionTexto7", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Verificar("statusText7", statusText)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        if modoStatus then
            Verificar("Mode7", tostring(modoStatus.value))
        end

        if _storage.get_number("accion7") == ACCION_LIBRE  or  _storage.get_number("accion7") == ACCION_CICLO_APAGADO then
            Rutina_Encendido()
        end

        ActualizarAccion(ACCION_PABIERTA)
        Verificar("timerAccion7", "AutoOff")
        Verificar("scanCycle7", "AutoOff")
    ...
    if SensorPuerta.value == false then
        CancelTimer()
        if _storage.get_number("accion7") == ACCION_PABIERTA then

            if _storage.get_string("statusText7") == "Libre" then
                Rutina_Encendido()
            end

            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle7", "scan_7")
            Verificar("timerAccion7", "Scan")
            StartTimer(Tiempo1)
            return true
        else
            ...
            for i in pairs(sensoresMovimiento) do
                ...
                if sensor.value == true and SensorPuerta.value == false then
                    ...
                    if _storage.get_number("accion7") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion7", "Ocupado")
                        Verificar("scanCycle7", "Ocupado")
        ...
        for i in ipairs(Actuador) do
            ...
            if actuador_id.value == true then
                if data.scancycle == nil or data.scancycle == "Libre" then
                    ...
                    Verificar("scanCycle7", "Ocupado")
                    ActualizarAccion(ACCION_OCUPADO)
                    Registro("Pasa a accion 7 OCUPADO por disparo de Actuador true", amarillo)
                    return
                end
                if _storage.get_number("accion7") == ACCION_PABIERTA and data.scancycle == "PuertaAbierta" then
                    if _storage.get_string("statusText7") == "Libre" then
                        if masterswitch == "si" then
                            RoutineOn(devicesOn)
                        end
                ...
                if (tiempoRestante1 >= 10 and data.scancycle == "scan_7") then
                    ...
                    if _storage.get_number("accion7") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion7", "Ocupado")
                        Verificar("scanCycle7", "Ocupado")
                        Registro("se paso a accion7 OCUPADO por disparo de Actuador", verde)
                        return
                    end
                end
            ...
            if actuador_id.value == false then
                if _storage.get_number("accion7") == ACCION_SCAN then
                    ...
                    Verificar("timerAccion7","Ocupado")
                    Verificar("scanCycle7", "Ocupado")
                    Registro("Pasa a accion 7 OCUPADO por disparo de Actuador false", amarillo)
                    return
                end              
                if _storage.get_number("accion7") == ACCION_PABIERTA and data.statustext == "Ocupado" then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Verificar("timerAccion7", "Scan")
                    Verificar("scanCycle7", "scan_7")
...
function PuertaAbierta()
    ...
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Verificar("timerAccion7", "PuertaAbierta")
    Verificar("scanCycle7", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then
        ...
        --     offpuertaabierta .. ", " .. _storage.get_string("Mode7")
        -- Registro(Mensaje2, amarillo)
        CancelTimer()
        Verificar("statusText7", "Libre")
        ...
        -- if _storage.get_string("Mode7") ~= MODO_APAGADO and offpuertaabierta == "si" then
...
function CycleOff()
    ...
    if (contador == contadorLibres) then
        if data.scancycle == "scan_7" then
            CancelTimer()
            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle7", "scan_Libre")
            Verificar("timerAccion7", "Scan")
            StartTimer(Tiempo2)
        ...
        elseif data.scancycle == "scan_Libre" then
            ...
            Verificar("timerAccion7", "CicloApagado")
            Verificar("scanCycle7", "CicloApagado")
...
function Rutina_Encendido()
    ...
    local mode = _storage.get_string("Mode7")
...
function RoutineOn(item_id)
    local id = {}
        if _storage.get_string("Mode7") == MODO_AUTO then
...
function Main()
    ...
    Registro("previousTimer_7: " .. _json.encode(data.previousTimer))
    Registro("TimerRemainingV_7: " .. _json.encode(TimerRemainingV))
    Registro("Modo plugin: " .. _json.encode(data.modo),verde)
    Registro("TimerAccion7: " .. _json.encode(data.timeraccion),amarillo)
    Registro("scanCycle7: " .. _json.encode(data.scancycle),verde)
    ...
    Verificar("previousTimer7", tonumber(TimerRemainingV))

    if data.accion == 100 or data.accion == 0 then
        Verificar("TimerDuration7", 0)
    else
...
function StartTimer(timerduration)
    ...
    Verificar("TimerDuration7", timerduration)
...
function StartTimeralways()
    ...
    Verificar("dueTimestamp7", dueTimestamp1)
    local status = RemainingUpgrade()
    Verificar("Counting7", "7")
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID7", tostring(timerID))
        --Registro("TimerID7: " .. timerID) -- No es necesario mostrar el ID del Timer
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/functions/roomSeven",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/functions/roomSeven",
            { arg_name = "timer" })
        _storage.set_string("TimerID7", tostring(timerID))
        return true
    end
...
function CancelTimer()
    ...
    if data.TimerID ~= "" then
        local timerID = data.TimerID
        _storage.set_string("TimerID7", tostring(timerID))
        --Registro("TimerID7: " .. timerID, amarillo) -- No es necesario mostrar el ID del Timer
        if timerID then
    ...
    Remaining_ant = 0
    Verificar("Counting7", "0")
    Verificar("dueTimestamp7", 0)
    Verificar("remaining7", "0")
    Verificar("TimerDuration7", 0)
    return true
end
...
function RemainingUpgrade()
    ...
    Verificar("remaining7", "TR" .. restante)
    return remaining > 0
end
...
if modoPluginStatus then
    ...
    if modeButton.value ~= data.modo then
        Verificar("Mode7", tostring(modeButton.value))
...
if AccionPluginStatus then
    ...
    if AccionButton.value == "Libre" then
        if EstadoTexto.value == "Ocupado" then
            ...
            Verificar("scanCycle7", "LibreForzado")
            Libre()
        end
    end
end
