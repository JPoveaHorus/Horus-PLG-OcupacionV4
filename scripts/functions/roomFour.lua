...
local sensor_puerta = _storage.get_string("SENSOR_PUERTA4")
local devicesOn = _storage.get_table("ACTUADORES_ON4")
local devicesOff = _storage.get_table("ACTUADORES_OFF4") or {}
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT4")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC4")
local thermostat = _storage.get_table("TERMOSTATO4")
local idmasterswitch = _storage.get_string("MASTERSWITCH4")
...
local offpuertaabierta = _storage.get_string("OFFPUERTAABIERTA4")
local masterswitch = _storage.get_string("MASTERSWITCH4") or {}
local motionActivator = _storage.get_string("MOTIONACTIVATOR4")
local SetPointOn = _storage.get_number("SETPOINTON4")
local SetPointOff = _storage.get_number("SETPOINTOFF4")
local ModoSetpoint = _storage.get_string("MODOSETPOINT4")

local data = {}
data.modo = _storage.get_string("Mode4")
...
----- Tiempos del Plugin
local Tiemposensor = _storage.get_number("TIEMPOSCAN4")
...
function ActualizarAccion(s)
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Verificar("accionTexto4", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Verificar("statusText4", statusText)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        if modoStatus then
            Verificar("Mode4", tostring(modoStatus.value))
        end

        if _storage.get_number("accion4") == ACCION_LIBRE  or  _storage.get_number("accion4") == ACCION_CICLO_APAGADO then
            Rutina_Encendido()
        end

        ActualizarAccion(ACCION_PABIERTA)
        Verificar("timerAccion4", "AutoOff")
        Verificar("scanCycle4", "AutoOff")
    ...
    if SensorPuerta.value == false then
        CancelTimer()
        if _storage.get_number("accion4") == ACCION_PABIERTA then

            if _storage.get_string("statusText4") == "Libre" then
                Rutina_Encendido()
            end

            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle4", "scan_4")
            Verificar("timerAccion4", "Scan")
            StartTimer(Tiempo1)
            return true
        else
            ...
            for i in pairs(sensoresMovimiento) do
                ...
                if sensor.value == true and SensorPuerta.value == false then
                    ...
                    if _storage.get_number("accion4") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion4", "Ocupado")
                        Verificar("scanCycle4", "Ocupado")
        ...
        for i in ipairs(Actuador) do
            ...
            if actuador_id.value == true then
                if data.scancycle == nil or data.scancycle == "Libre" then
                    ...
                    Verificar("scanCycle4", "Ocupado")
                    ActualizarAccion(ACCION_OCUPADO)
                    Registro("Pasa a accion 4 OCUPADO por disparo de Actuador true", amarillo)
                    return
                end
                if _storage.get_number("accion4") == ACCION_PABIERTA and data.scancycle == "PuertaAbierta" then
                    if _storage.get_string("statusText4") == "Libre" then
                        if masterswitch == "si" then
                            RoutineOn(devicesOn)
                        end
                ...
                if (tiempoRestante1 >= 10 and data.scancycle == "scan_4") then
                    ...
                    if _storage.get_number("accion4") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion4", "Ocupado")
                        Verificar("scanCycle4", "Ocupado")
                        Registro("se paso a accion4 OCUPADO por disparo de Actuador", verde)
                        return
                    end
                end
            ...
            if actuador_id.value == false then
                if _storage.get_number("accion4") == ACCION_SCAN then
                    ...
                    Verificar("timerAccion4","Ocupado")
                    Verificar("scanCycle4", "Ocupado")
                    Registro("Pasa a accion 4 OCUPADO por disparo de Actuador false", amarillo)
                    return
                end              
                if _storage.get_number("accion4") == ACCION_PABIERTA and data.statustext == "Ocupado" then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Verificar("timerAccion4", "Scan")
                    Verificar("scanCycle4", "scan_4")
...
function PuertaAbierta()
    ...
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Verificar("timerAccion4", "PuertaAbierta")
    Verificar("scanCycle4", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then
        ...
        --     offpuertaabierta .. ", " .. _storage.get_string("Mode4")
        -- Registro(Mensaje2, amarillo)
        CancelTimer()
        Verificar("statusText4", "Libre")
        ...
        -- if _storage.get_string("Mode4") ~= MODO_APAGADO and offpuertaabierta == "si" then
...
function CycleOff()
    ...
    if (contador == contadorLibres) then
        if data.scancycle == "scan_4" then
            CancelTimer()
            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle4", "scan_Libre")
            Verificar("timerAccion4", "Scan")
            StartTimer(Tiempo2)
        ...
        elseif data.scancycle == "scan_Libre" then
            ...
            Verificar("timerAccion4", "CicloApagado")
            Verificar("scanCycle4", "CicloApagado")
...
function Rutina_Encendido()
    ...
    local mode = _storage.get_string("Mode4")
...
function RoutineOn(item_id)
    local id = {}
        if _storage.get_string("Mode4") == MODO_AUTO then
...
function Main()
    ...
    Registro("previousTimer_4: " .. _json.encode(data.previousTimer))
    Registro("TimerRemainingV_4: " .. _json.encode(TimerRemainingV))
    Registro("Modo plugin: " .. _json.encode(data.modo),verde)
    Registro("TimerAccion4: " .. _json.encode(data.timeraccion),amarillo)
    Registro("scanCycle4: " .. _json.encode(data.scancycle),verde)
    ...
    Verificar("previousTimer4", tonumber(TimerRemainingV))

    if data.accion == 100 or data.accion == 0 then
        Verificar("TimerDuration4", 0)
    else
...
function StartTimer(timerduration)
    ...
    Verificar("TimerDuration4", timerduration)
...
function StartTimeralways()
    ...
    Verificar("dueTimestamp4", dueTimestamp1)
    local status = RemainingUpgrade()
    Verificar("Counting4", "4")
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID4", tostring(timerID))
        --Registro("TimerID4: " .. timerID) -- No es necesario mostrar el ID del Timer
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/functions/roomFour",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/functions/roomFour",
            { arg_name = "timer" })
        _storage.set_string("TimerID4", tostring(timerID))
        return true
    end
...
function CancelTimer()
    ...
    if data.TimerID ~= "" then
        local timerID = data.TimerID
        _storage.set_string("TimerID4", tostring(timerID))
        --Registro("TimerID4: " .. timerID, amarillo) -- No es necesario mostrar el ID del Timer
        if timerID then
    ...
    Remaining_ant = 0
    Verificar("Counting4", "0")
    Verificar("dueTimestamp4", 0)
    Verificar("remaining4", "0")
    Verificar("TimerDuration4", 0)
    return true
end
...
function RemainingUpgrade()
    ...
    Verificar("remaining4", "TR" .. restante)
    return remaining > 0
end
...
if modoPluginStatus then
    ...
    if modeButton.value ~= data.modo then
        Verificar("Mode4", tostring(modeButton.value))
...
if AccionPluginStatus then
    ...
    if AccionButton.value == "Libre" then
        if EstadoTexto.value == "Ocupado" then
            ...
            Verificar("scanCycle4", "LibreForzado")
            Libre()
        end
    end
end
