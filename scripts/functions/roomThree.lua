...
local sensor_puerta = _storage.get_string("SENSOR_PUERTA3")
local devicesOn = _storage.get_table("ACTUADORES_ON3")
local devicesOff = _storage.get_table("ACTUADORES_OFF3") or {}
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT3")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC3")
local thermostat = _storage.get_table("TERMOSTATO3")
local idmasterswitch = _storage.get_string("MASTERSWITCH3")
...
local offpuertaabierta = _storage.get_string("OFFPUERTAABIERTA3")
local masterswitch = _storage.get_string("MASTERSWITCH3") or {}
local motionActivator = _storage.get_string("MOTIONACTIVATOR3")
local SetPointOn = _storage.get_number("SETPOINTON3")
local SetPointOff = _storage.get_number("SETPOINTOFF3")
local ModoSetpoint = _storage.get_string("MODOSETPOINT3")

local data = {}
data.modo = _storage.get_string("Mode3")
...
----- Tiempos del Plugin
local Tiemposensor = _storage.get_number("TIEMPOSCAN3")
...
function ActualizarAccion(s)
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Verificar("accionTexto3", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Verificar("statusText3", statusText)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        if modoStatus then
            Verificar("Mode3", tostring(modoStatus.value))
        end
        ...
        Verificar("timerAccion3", "AutoOff")
        Verificar("scanCycle3", "AutoOff")
    ...
    if SensorPuerta.value == false then
        ...
        if _storage.get_number("accion3") == ACCION_PABIERTA then
            ...
            Verificar("scanCycle3", "scan_3")
            Verificar("timerAccion3", "Scan")
            StartTimer(Tiempo1)
            return true
        else
            ...
            for i in pairs(sensoresMovimiento) do
                ...
                if sensor.value == true and SensorPuerta.value == false then
                    ...
                    if _storage.get_number("accion3") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion3", "Ocupado")
                        Verificar("scanCycle3", "Ocupado")
    ...
    if Actuador ~= nil then
        ...
        for i in ipairs(Actuador) do
            ...
            if actuador_id.value == true then
                ...
                if _storage.get_number("accion3") == ACCION_PABIERTA and data.scancycle == "PuertaAbierta" then
                    if _storage.get_string("statusText3") == "Libre" then
                        if masterswitch == "si" then
                            RoutineOn(devicesOn)
                        end
                ...
                if (tiempoRestante1 >= 10 and data.scancycle == "scan_3") then
            ...
            if actuador_id.value == false then
                if _storage.get_number("accion3") == ACCION_SCAN then
                    ...
                    Verificar("timerAccion3","Ocupado")
                    Verificar("scanCycle3", "Ocupado")
                    Registro("Pasa a accion 3 OCUPADO por disparo de Actuador false", amarillo)
                    return
                end              
                if _storage.get_number("accion3") == ACCION_PABIERTA and data.statustext == "Ocupado" then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Verificar("timerAccion3", "Scan")
                    Verificar("scanCycle3", "scan_3")
...
function PuertaAbierta()
    ...
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Verificar("timerAccion3", "PuertaAbierta")
    Verificar("scanCycle3", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then
        ...
        --     offpuertaabierta .. ", " .. _storage.get_string("Mode3")
        -- Registro(Mensaje2, amarillo)
        CancelTimer()
        Verificar("statusText3", "Libre")
        ...
        -- if _storage.get_string("Mode3") ~= MODO_APAGADO and offpuertaabierta == "si" then
...
function CycleOff()
    ...
    if (contador == contadorLibres) then
        if data.scancycle == "scan_3" then
            CancelTimer()
            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle3", "scan_Libre")
            Verificar("timerAccion3", "Scan")
            StartTimer(Tiempo2)
        ...
        elseif data.scancycle == "scan_Libre" then
            ...
            Verificar("timerAccion3", "CicloApagado")
            Verificar("scanCycle3", "CicloApagado")
...
function Rutina_Encendido()
    ...
    local mode = _storage.get_string("Mode3")
...
function RoutineOn(item_id)
    local id = {}
        if _storage.get_string("Mode3") == MODO_AUTO then
...
function Main()
    ...
    Registro("previousTimer_3: " .. _json.encode(data.previousTimer))
    Registro("TimerRemainingV_3: " .. _json.encode(TimerRemainingV))
    Registro("Modo plugin: " .. _json.encode(data.modo),verde)
    Registro("TimerAccion3: " .. _json.encode(data.timeraccion),amarillo)
    Registro("scanCycle3: " .. _json.encode(data.scancycle),verde)
    ...
    Verificar("previousTimer3", tonumber(TimerRemainingV))

    if data.accion == 100 or data.accion == 0 then
        Verificar("TimerDuration3", 0)
    else
...
function StartTimer(timerduration)
    ...
    Verificar("TimerDuration3", timerduration)
...
function StartTimeralways()
    ...
    Verificar("dueTimestamp3", dueTimestamp1)
    local status = RemainingUpgrade()
    Verificar("Counting3", "3")
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID3", tostring(timerID))
        --Registro("TimerID3: " .. timerID) -- No es necesario mostrar el ID del Timer
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/functions/roomThree",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/functions/roomThree",
            { arg_name = "timer" })
        _storage.set_string("TimerID3", tostring(timerID))
        return true
    end
...
function CancelTimer()
    ...
    if data.TimerID ~= "" then
        local timerID = data.TimerID
        _storage.set_string("TimerID3", tostring(timerID))
        --Registro("TimerID3: " .. timerID, amarillo) -- No es necesario mostrar el ID del Timer
        if timerID then
    ...
    Remaining_ant = 0
    Verificar("Counting3", "0")
    Verificar("dueTimestamp3", 0)
    Verificar("remaining3", "0")
    Verificar("TimerDuration3", 0)
    return true
end
...
function RemainingUpgrade()
    ...
    Verificar("remaining3", "TR" .. restante)
    return remaining > 0
end
...
if modoPluginStatus then
    ...
    if modeButton.value ~= data.modo then
        Verificar("Mode3", tostring(modeButton.value))
...
if AccionPluginStatus then
    ...
    if AccionButton.value == "Libre" then
        if EstadoTexto.value == "Ocupado" then
            ...
            Verificar("scanCycle3", "LibreForzado")
            Libre()
        end
    end
end
