...
local sensor_puerta = _storage.get_string("SENSOR_PUERTA2")
local devicesOn = _storage.get_table("ACTUADORES_ON2")
local devicesOff = _storage.get_table("ACTUADORES_OFF2") or {}
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT2")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC2")
local thermostat = _storage.get_table("TERMOSTATO2")
local idmasterswitch = _storage.get_string("MASTERSWITCH2")
...
local offpuertaabierta = _storage.get_string("OFFPUERTAABIERTA2")
local masterswitch = _storage.get_string("MASTERSWITCH2") or {}
local motionActivator = _storage.get_string("MOTIONACTIVATOR2")
local SetPointOn = _storage.get_number("SETPOINTON2")
local SetPointOff = _storage.get_number("SETPOINTOFF2")
local ModoSetpoint = _storage.get_string("MODOSETPOINT2")

local data = {}
data.modo = _storage.get_string("Mode2")
...
----- Tiempos del Plugin
local Tiemposensor = _storage.get_number("TIEMPOSCAN2")
...
function ActualizarAccion(s)
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Verificar("accionTexto2", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Verificar("statusText2", statusText)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        if modoStatus then
            Verificar("Mode2", tostring(modoStatus.value))
        end

        if _storage.get_number("accion2") == ACCION_LIBRE  or  _storage.get_number("accion2") == ACCION_CICLO_APAGADO then
            Rutina_Encendido()
        end

        ActualizarAccion(ACCION_PABIERTA)
        Verificar("timerAccion2", "AutoOff")
        Verificar("scanCycle2", "AutoOff")
    ...
    if SensorPuerta.value == false then
        CancelTimer()
        if _storage.get_number("accion2") == ACCION_PABIERTA then

            if _storage.get_string("statusText2") == "Libre" then
                Rutina_Encendido()
            end

            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle2", "scan_2")
            Verificar("timerAccion2", "Scan")
            StartTimer(Tiempo1)
            return true
        else
            ...
            for i in pairs(sensoresMovimiento) do
                ...
                if sensor.value == true and SensorPuerta.value == false then
                    ...
                    if _storage.get_number("accion2") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion2", "Ocupado")
                        Verificar("scanCycle2", "Ocupado")
        ...
        for i in ipairs(Actuador) do
            ...
            if actuador_id.value == true then
                if data.scancycle == nil or data.scancycle == "Libre" then
                    ...
                    Verificar("scanCycle2", "Ocupado")
                    ActualizarAccion(ACCION_OCUPADO)
                    Registro("Pasa a accion 2 OCUPADO por disparo de Actuador true", amarillo)
                    return
                end
                if _storage.get_number("accion2") == ACCION_PABIERTA and data.scancycle == "PuertaAbierta" then
                    if _storage.get_string("statusText2") == "Libre" then
                        if masterswitch == "si" then
                            RoutineOn(devicesOn)
                        end
                ...
                if (tiempoRestante1 >= 10 and data.scancycle == "scan_2") then
                    ...
                    if _storage.get_number("accion2") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion2", "Ocupado")
                        Verificar("scanCycle2", "Ocupado")
                        Registro("se paso a accion2 OCUPADO por disparo de Actuador", verde)
                        return
                    end
                end
            ...
            if actuador_id.value == false then
                if _storage.get_number("accion2") == ACCION_SCAN then
                    ...
                    Verificar("timerAccion2","Ocupado")
                    Verificar("scanCycle2", "Ocupado")
                    Registro("Pasa a accion 2 OCUPADO por disparo de Actuador false", amarillo)
                    return
                end              
                if _storage.get_number("accion2") == ACCION_PABIERTA and data.statustext == "Ocupado" then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Verificar("timerAccion2", "Scan")
                    Verificar("scanCycle2", "scan_2")
...
function PuertaAbierta()
    ...
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Verificar("timerAccion2", "PuertaAbierta")
    Verificar("scanCycle2", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then
        ...
        --     offpuertaabierta .. ", " .. _storage.get_string("Mode2")
        -- Registro(Mensaje2, amarillo)
        CancelTimer()
        Verificar("statusText2", "Libre")
        ...
        -- if _storage.get_string("Mode2") ~= MODO_APAGADO and offpuertaabierta == "si" then
...
function CycleOff()
    ...
    if (contador == contadorLibres) then
        if data.scancycle == "scan_2" then
            CancelTimer()
            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle2", "scan_Libre")
            Verificar("timerAccion2", "Scan")
            StartTimer(Tiempo2)
        ...
        elseif data.scancycle == "scan_Libre" then
            ...
            Verificar("timerAccion2", "CicloApagado")
            Verificar("scanCycle2", "CicloApagado")
...
function Rutina_Encendido()
    ...
    local mode = _storage.get_string("Mode2")
...
function RoutineOn(item_id)
    local id = {}
        if _storage.get_string("Mode2") == MODO_AUTO then
...
function Main()
    ...
    Registro("previousTimer_2: " .. _json.encode(data.previousTimer))
    Registro("TimerRemainingV_2: " .. _json.encode(TimerRemainingV))
    Registro("Modo plugin: " .. _json.encode(data.modo),verde)
    Registro("TimerAccion2: " .. _json.encode(data.timeraccion),amarillo)
    Registro("scanCycle2: " .. _json.encode(data.scancycle),verde)
    ...
    Verificar("previousTimer2", tonumber(TimerRemainingV))

    if data.accion == 100 or data.accion == 0 then
        Verificar("TimerDuration2", 0)
    else
...
function StartTimer(timerduration)
    ...
    Verificar("TimerDuration2", timerduration)
...
function StartTimeralways()
    ...
    Verificar("dueTimestamp2", dueTimestamp1)
    local status = RemainingUpgrade()
    Verificar("Counting2", "2")
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID2", tostring(timerID))
        --Registro("TimerID2: " .. timerID) -- No es necesario mostrar el ID del Timer
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/functions/roomTwo",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/functions/roomTwo",
            { arg_name = "timer" })
        _storage.set_string("TimerID2", tostring(timerID))
        return true
    end
...
function CancelTimer()
    ...
    if data.TimerID ~= "" then
        local timerID = data.TimerID
        _storage.set_string("TimerID2", tostring(timerID))
        --Registro("TimerID2: " .. timerID, amarillo) -- No es necesario mostrar el ID del Timer
        if timerID then
    ...
    Remaining_ant = 0
    Verificar("Counting2", "0")
    Verificar("dueTimestamp2", 0)
    Verificar("remaining2", "0")
    Verificar("TimerDuration2", 0)
    return true
end
...
function RemainingUpgrade()
    ...
    Verificar("remaining2", "TR" .. restante)
    return remaining > 0
end
...
if modoPluginStatus then
    ...
    if modeButton.value ~= data.modo then
        Verificar("Mode2", tostring(modeButton.value))
...
if AccionPluginStatus then
    ...
    if AccionButton.value == "Libre" then
        if EstadoTexto.value == "Ocupado" then
            ...
            Verificar("scanCycle2", "LibreForzado")
            Libre()
        end
    end
end
