...
local sensor_puerta = _storage.get_string("SENSOR_PUERTA5")
local devicesOn = _storage.get_table("ACTUADORES_ON5")
local devicesOff = _storage.get_table("ACTUADORES_OFF5") or {}
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT5")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC5")
local thermostat = _storage.get_table("TERMOSTATO5")
local idmasterswitch = _storage.get_string("MASTERSWITCH5")
...
local offpuertaabierta = _storage.get_string("OFFPUERTAABIERTA5")
local masterswitch = _storage.get_string("MASTERSWITCH5") or {}
local motionActivator = _storage.get_string("MOTIONACTIVATOR5")
local SetPointOn = _storage.get_number("SETPOINTON5")
local SetPointOff = _storage.get_number("SETPOINTOFF5")
local ModoSetpoint = _storage.get_string("MODOSETPOINT5")

local data = {}
data.modo = _storage.get_string("Mode5")
...
----- Tiempos del Plugin
local Tiemposensor = _storage.get_number("TIEMPOSCAN5")
...
function ActualizarAccion(s)
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Verificar("accionTexto5", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Verificar("statusText5", statusText)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        if modoStatus then
            Verificar("Mode5", tostring(modoStatus.value))
        end

        if _storage.get_number("accion5") == ACCION_LIBRE  or  _storage.get_number("accion5") == ACCION_CICLO_APAGADO then
            Rutina_Encendido()
        end

        ActualizarAccion(ACCION_PABIERTA)
        Verificar("timerAccion5", "AutoOff")
        Verificar("scanCycle5", "AutoOff")
    ...
    if SensorPuerta.value == false then
        CancelTimer()
        if _storage.get_number("accion5") == ACCION_PABIERTA then

            if _storage.get_string("statusText5") == "Libre" then
                Rutina_Encendido()
            end

            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle5", "scan_5")
            Verificar("timerAccion5", "Scan")
            StartTimer(Tiempo1)
            return true
        else
            ...
            for i in pairs(sensoresMovimiento) do
                ...
                if sensor.value == true and SensorPuerta.value == false then
                    ...
                    if _storage.get_number("accion5") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion5", "Ocupado")
                        Verificar("scanCycle5", "Ocupado")
        ...
        for i in ipairs(Actuador) do
            ...
            if actuador_id.value == true then
                if data.scancycle == nil or data.scancycle == "Libre" then
                    ...
                    Verificar("scanCycle5", "Ocupado")
                    ActualizarAccion(ACCION_OCUPADO)
                    Registro("Pasa a accion 5 OCUPADO por disparo de Actuador true", amarillo)
                    return
                end
                if _storage.get_number("accion5") == ACCION_PABIERTA and data.scancycle == "PuertaAbierta" then
                    if _storage.get_string("statusText5") == "Libre" then
                        if masterswitch == "si" then
                            RoutineOn(devicesOn)
                        end
                ...
                if (tiempoRestante1 >= 10 and data.scancycle == "scan_5") then
                    ...
                    if _storage.get_number("accion5") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion5", "Ocupado")
                        Verificar("scanCycle5", "Ocupado")
                        Registro("se paso a accion5 OCUPADO por disparo de Actuador", verde)
                        return
                    end
                end
            ...
            if actuador_id.value == false then
                if _storage.get_number("accion5") == ACCION_SCAN then
                    ...
                    Verificar("timerAccion5","Ocupado")
                    Verificar("scanCycle5", "Ocupado")
                    Registro("Pasa a accion 5 OCUPADO por disparo de Actuador false", amarillo)
                    return
                end              
                if _storage.get_number("accion5") == ACCION_PABIERTA and data.statustext == "Ocupado" then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Verificar("timerAccion5", "Scan")
                    Verificar("scanCycle5", "scan_5")
...
function PuertaAbierta()
    ...
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Verificar("timerAccion5", "PuertaAbierta")
    Verificar("scanCycle5", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then
        ...
        --     offpuertaabierta .. ", " .. _storage.get_string("Mode5")
        -- Registro(Mensaje2, amarillo)
        CancelTimer()
        Verificar("statusText5", "Libre")
        ...
        -- if _storage.get_string("Mode5") ~= MODO_APAGADO and offpuertaabierta == "si" then
...
function CycleOff()
    ...
    if (contador == contadorLibres) then
        if data.scancycle == "scan_5" then
            CancelTimer()
            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle5", "scan_Libre")
            Verificar("timerAccion5", "Scan")
            StartTimer(Tiempo2)
        ...
        elseif data.scancycle == "scan_Libre" then
            ...
            Verificar("timerAccion5", "CicloApagado")
            Verificar("scanCycle5", "CicloApagado")
...
function Rutina_Encendido()
    ...
    local mode = _storage.get_string("Mode5")
...
function RoutineOn(item_id)
    local id = {}
        if _storage.get_string("Mode5") == MODO_AUTO then
...
function Main()
    ...
    Registro("previousTimer_5: " .. _json.encode(data.previousTimer))
    Registro("TimerRemainingV_5: " .. _json.encode(TimerRemainingV))
    Registro("Modo plugin: " .. _json.encode(data.modo),verde)
    Registro("TimerAccion5: " .. _json.encode(data.timeraccion),amarillo)
    Registro("scanCycle5: " .. _json.encode(data.scancycle),verde)
    ...
    Verificar("previousTimer5", tonumber(TimerRemainingV))

    if data.accion == 100 or data.accion == 0 then
        Verificar("TimerDuration5", 0)
    else
...
function StartTimer(timerduration)
    ...
    Verificar("TimerDuration5", timerduration)
...
function StartTimeralways()
    ...
    Verificar("dueTimestamp5", dueTimestamp1)
    local status = RemainingUpgrade()
    Verificar("Counting5", "5")
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID5", tostring(timerID))
        --Registro("TimerID5: " .. timerID) -- No es necesario mostrar el ID del Timer
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/functions/roomFive",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/functions/roomFive",
            { arg_name = "timer" })
        _storage.set_string("TimerID5", tostring(timerID))
        return true
    end
...
function CancelTimer()
    ...
    if data.TimerID ~= "" then
        local timerID = data.TimerID
        _storage.set_string("TimerID5", tostring(timerID))
        --Registro("TimerID5: " .. timerID, amarillo) -- No es necesario mostrar el ID del Timer
        if timerID then
    ...
    Remaining_ant = 0
    Verificar("Counting5", "0")
    Verificar("dueTimestamp5", 0)
    Verificar("remaining5", "0")
    Verificar("TimerDuration5", 0)
    return true
end
...
function RemainingUpgrade()
    ...
    Verificar("remaining5", "TR" .. restante)
    return remaining > 0
end
...
if modoPluginStatus then
    ...
    if modeButton.value ~= data.modo then
        Verificar("Mode5", tostring(modeButton.value))
...
if AccionPluginStatus then
    ...
    if AccionButton.value == "Libre" then
        if EstadoTexto.value == "Ocupado" then
            ...
            Verificar("scanCycle5", "LibreForzado")
            Libre()
        end
    end
end
