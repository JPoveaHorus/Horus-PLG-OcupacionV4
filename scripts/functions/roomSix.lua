...
local sensor_puerta = _storage.get_string("SENSOR_PUERTA6")
local devicesOn = _storage.get_table("ACTUADORES_ON6")
local devicesOff = _storage.get_table("ACTUADORES_OFF6") or {}
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT6")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC6")
local thermostat = _storage.get_table("TERMOSTATO6")
local idmasterswitch = _storage.get_string("MASTERSWITCH6")
...
local offpuertaabierta = _storage.get_string("OFFPUERTAABIERTA6")
local masterswitch = _storage.get_string("MASTERSWITCH6") or {}
local motionActivator = _storage.get_string("MOTIONACTIVATOR6")
local SetPointOn = _storage.get_number("SETPOINTON6")
local SetPointOff = _storage.get_number("SETPOINTOFF6")
local ModoSetpoint = _storage.get_string("MODOSETPOINT6")

local data = {}
data.modo = _storage.get_string("Mode6")
...
----- Tiempos del Plugin
local Tiemposensor = _storage.get_number("TIEMPOSCAN6")
...
function ActualizarAccion(s)
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Verificar("accionTexto6", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Verificar("statusText6", statusText)
...
function EventoPuerta()
    ...
    if SensorPuerta.value == true then
        ...
        if modoStatus then
            Verificar("Mode6", tostring(modoStatus.value))
        end

        if _storage.get_number("accion6") == ACCION_LIBRE  or  _storage.get_number("accion6") == ACCION_CICLO_APAGADO then
            Rutina_Encendido()
        end

        ActualizarAccion(ACCION_PABIERTA)
        Verificar("timerAccion6", "AutoOff")
        Verificar("scanCycle6", "AutoOff")
    ...
    if SensorPuerta.value == false then
        CancelTimer()
        if _storage.get_number("accion6") == ACCION_PABIERTA then

            if _storage.get_string("statusText6") == "Libre" then
                Rutina_Encendido()
            end

            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle6", "scan_6")
            Verificar("timerAccion6", "Scan")
            StartTimer(Tiempo1)
            return true
        else
            ...
            for i in pairs(sensoresMovimiento) do
                ...
                if sensor.value == true and SensorPuerta.value == false then
                    ...
                    if _storage.get_number("accion6") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion6", "Ocupado")
                        Verificar("scanCycle6", "Ocupado")
        ...
        for i in ipairs(Actuador) do
            ...
            if actuador_id.value == true then
                if data.scancycle == nil or data.scancycle == "Libre" then
                    ...
                    Verificar("scanCycle6", "Ocupado")
                    ActualizarAccion(ACCION_OCUPADO)
                    Registro("Pasa a accion 6 OCUPADO por disparo de Actuador true", amarillo)
                    return
                end
                if _storage.get_number("accion6") == ACCION_PABIERTA and data.scancycle == "PuertaAbierta" then
                    if _storage.get_string("statusText6") == "Libre" then
                        if masterswitch == "si" then
                            RoutineOn(devicesOn)
                        end
                ...
                if (tiempoRestante1 >= 10 and data.scancycle == "scan_6") then
                    ...
                    if _storage.get_number("accion6") == ACCION_SCAN then
                        ...
                        Verificar("timerAccion6", "Ocupado")
                        Verificar("scanCycle6", "Ocupado")
                        Registro("se paso a accion6 OCUPADO por disparo de Actuador", verde)
                        return
                    end
                end
            ...
            if actuador_id.value == false then
                if _storage.get_number("accion6") == ACCION_SCAN then
                    ...
                    Verificar("timerAccion6","Ocupado")
                    Verificar("scanCycle6", "Ocupado")
                    Registro("Pasa a accion 6 OCUPADO por disparo de Actuador false", amarillo)
                    return
                end              
                if _storage.get_number("accion6") == ACCION_PABIERTA and data.statustext == "Ocupado" then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Verificar("timerAccion6", "Scan")
                    Verificar("scanCycle6", "scan_6")
...
function PuertaAbierta()
    ...
    CancelTimer()
    ActualizarAccion(ACCION_PABIERTA)
    Verificar("timerAccion6", "PuertaAbierta")
    Verificar("scanCycle6", "PuertaAbierta")
...
function PuertaAbiertaLibre()
    ...
    if contador == contadorLibres then
        ...
        --     offpuertaabierta .. ", " .. _storage.get_string("Mode6")
        -- Registro(Mensaje2, amarillo)
        CancelTimer()
        Verificar("statusText6", "Libre")
        ...
        -- if _storage.get_string("Mode6") ~= MODO_APAGADO and offpuertaabierta == "si" then
...
function CycleOff()
    ...
    if (contador == contadorLibres) then
        if data.scancycle == "scan_6" then
            CancelTimer()
            ActualizarAccion(ACCION_SCAN)
            Verificar("scanCycle6", "scan_Libre")
            Verificar("timerAccion6", "Scan")
            StartTimer(Tiempo2)
        ...
        elseif data.scancycle == "scan_Libre" then
            ...
            Verificar("timerAccion6", "CicloApagado")
            Verificar("scanCycle6", "CicloApagado")
...
function Rutina_Encendido()
    ...
    local mode = _storage.get_string("Mode6")
...
function RoutineOn(item_id)
    local id = {}
        if _storage.get_string("Mode6") == MODO_AUTO then
...
function Main()
    ...
    Registro("previousTimer_6: " .. _json.encode(data.previousTimer))
    Registro("TimerRemainingV_6: " .. _json.encode(TimerRemainingV))
    Registro("Modo plugin: " .. _json.encode(data.modo),verde)
    Registro("TimerAccion6: " .. _json.encode(data.timeraccion),amarillo)
    Registro("scanCycle6: " .. _json.encode(data.scancycle),verde)
    ...
    Verificar("previousTimer6", tonumber(TimerRemainingV))

    if data.accion == 100 or data.accion == 0 then
        Verificar("TimerDuration6", 0)
    else
...
function StartTimer(timerduration)
    ...
    Verificar("TimerDuration6", timerduration)
...
function StartTimeralways()
    ...
    Verificar("dueTimestamp6", dueTimestamp1)
    local status = RemainingUpgrade()
    Verificar("Counting6", "6")
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID6", tostring(timerID))
        --Registro("TimerID6: " .. timerID) -- No es necesario mostrar el ID del Timer
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:plg.plugin_ocupacion/scripts/functions/roomSix",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:plg.plugin_ocupacion/scripts/functions/roomSix",
            { arg_name = "timer" })
        _storage.set_string("TimerID6", tostring(timerID))
        return true
    end
...
function CancelTimer()
    ...
    if data.TimerID ~= "" then
        local timerID = data.TimerID
        _storage.set_string("TimerID6", tostring(timerID))
        --Registro("TimerID6: " .. timerID, amarillo) -- No es necesario mostrar el ID del Timer
        if timerID then
    ...
    Remaining_ant = 0
    Verificar("Counting6", "0")
    Verificar("dueTimestamp6", 0)
    Verificar("remaining6", "0")
    Verificar("TimerDuration6", 0)
    return true
end
...
function RemainingUpgrade()
    ...
    Verificar("remaining6", "TR" .. restante)
    return remaining > 0
end
...
if modoPluginStatus then
    ...
    if modeButton.value ~= data.modo then
        Verificar("Mode6", tostring(modeButton.value))
...
if AccionPluginStatus then
    ...
    if AccionButton.value == "Libre" then
        if EstadoTexto.value == "Ocupado" then
            ...
            Verificar("scanCycle6", "LibreForzado")
            Libre()
        end
    end
end
